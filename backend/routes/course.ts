...
async function courseRoutes(
  ...
  // GET all courses
  fastify.get('/courses', async (_req: FastifyRequest, _reply: FastifyReply) => {
  ...
  // GET a course by ID
  fastify.get('/courses/:id', async (req: FastifyRequest, _reply: FastifyReply) => {
  ...
  fastify.post('/courses', async (req: FastifyRequest, _reply: FastifyReply) => {
  ...
  // PUT update a course
  fastify.patch('/courses/:id', async (req: FastifyRequest, _reply: FastifyReply) => {
    const { id } = req.params as { id: string };
    const { code, name, description } = req.body as { code?: string; name?: string; description?: string };
  ...
  // DELETE a course
  fastify.delete('/courses/:id', async (req: FastifyRequest, _reply: FastifyReply) => {
    const { id } = req.params as { id: string };
