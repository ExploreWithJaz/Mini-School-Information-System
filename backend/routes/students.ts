...
async function studentRoutes(
  ...
  // GET all students with optional search, filter, pagination - PROTECTED
  ...
  // PATCH update student - PROTECTED
  ...
  // GET eligible subjects for a student - PROTECTED
  fastify.get('/students/:id/eligible-subjects', 
    { onRequest: [authenticate] },
    async (req: FastifyRequest, _reply: FastifyReply) => {
      const { id } = req.params as { id: string };
  ...
  // POST /students/import - CSV bulk import - PROTECTED
  fastify.post<{ Body: any }>('/students/import', 
    { onRequest: [authenticate] },
